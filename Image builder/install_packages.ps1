...
$applications = @{
    ...
    "AZCLI"         = @{
        InstallerPath = Join-Path -Path $installRoot -ChildPath "azcli.msi"
        InstallArgs   = @('/i', 'c:\ImageBuilder\azcli.msi', '/quiet')
        TestPath      = "C:\Program Files\Microsoft SDKs\Azure\CLI2\wbin\az"
        DownloadUrl   = "https://storageaccountname.blob.core.windows.net/packages/azcli.msi"
    }
    "notepad++"     = @{
